...
class TrainSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(7):
            ...
            img_lr = Image.open(self.dir + '/LR_x2/' + self.train_list[idx] + '/im' + str(i + 1) + '.png')
...
class TestSetLoader(Dataset):
    def __init__(self, dataset_dir, scale_factor):
        ...
        self.img_list = os.listdir(self.dataset_dir + '/lr_x2')
        # get_key = lambda i: int(i.split('.')[0].split('_')[-1])
        # self.img_list = sorted(self.img_list, key=get_key)
        self.totensor = transforms.ToTensor()
    def __getitem__(self, idx):
        HR = []
        LR = []
        for idx_frame in range(idx - 3, idx + 4):   #第0帧：-3 -2 -1 0 1 2 3       0 0 0 0 1 2 3      1 1 1 1 2 3 4
            if idx_frame < 0:
                idx_frame = 0
            if idx_frame > len(self.img_list) - 1:
                idx_frame = len(self.img_list) - 1
            #为什么要用HR，应该是LR->HR    HR是ground truth 用于计算PSNR
            img_HR = Image.open(self.dataset_dir + '/hr/hr_' + str(idx_frame+1).rjust(2,'0') + '.png')
            img_LR = Image.open(self.dataset_dir + '/lr_x2/lr_' + str(idx_frame + 1).rjust(2, '0') + '.png')
            img_HR = np.array(img_HR, dtype=np.float32) / 255.0
            if idx_frame == idx:
                h, w, c = img_HR.shape
                SR_buicbic = np.array(img_LR.resize((w, h), Image.BICUBIC), dtype=np.float32) / 255.0   #SR_buicbic仅仅用于将最终的Y通道SR进行RGB可视化，cb和cr仅仅用bicubic插值得到
                SR_buicbic = rgb2ycbcr(SR_buicbic, only_y=False).transpose(2, 0, 1)
            img_LR = np.array(img_LR, dtype=np.float32) / 255.0
            img_HR = rgb2ycbcr(img_HR, only_y=True)[np.newaxis,:]   #仅转换亮度（Y）,前面增加一个维度是为了后面的append，也只利用Y通道
            img_LR = rgb2ycbcr(img_LR, only_y=True)[np.newaxis,:]
        ...
        C, N, H, W= HR.shape
        H = math.floor(H / self.upscale_factor / 4) * self.upscale_factor * 4    #为什么除两次4
...
class TestSetLoader_Vimeo(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for idx_frame in range(idx - 3, idx + 4):
            ...
            img_lr = Image.open(self.dir + '/LR_x2/' + self.video_name + '/im' + str(idx_frame + 1) + '.png')
            img_hr = np.array(img_hr, dtype=np.float32) / 255.0
            if idx_frame == idx:
                h, w, c = img_hr.shape
