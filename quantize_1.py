...
class ObserverBase(nn.Module):
    ...
    def forward(self, input):
        if self.q_level == "L":  # layer级(activation/weight)
            ...
            abs_value = torch.abs(input).view(-1)
        ...
        self.update_range(min_val, max_val, abs_value)
...
class MinMaxObserver(ObserverBase):
    ...
    def update_range(self, min_val_cur, max_val_cur, abs_value_cur):
        self.abs_value = abs_value_cur
        if self.q_level == "C":
...
class HistogramObserver(nn.Module):
    ...
    def forward(self, input):
        ...
        self.abs_value = torch.abs(input).view(-1)
        max_val_cur = torch.kthvalue(
            input.abs().contiguous().view(-1), int(self.percentile * input.contiguous().view(-1).size(0)), dim=0
        )[0]
        # MovingAverage
        if self.num_flag == 0:
            self.num_flag += 1
...
class Quantizer(nn.Module):
    ...
    def forward(self, input):
        if self.bits == 32:
            ...
            quant_result = torch.as_tensor(quant_result, dtype=torch.int8)

        return quant_result, self.scale
...
class SymmetricQuantizer(SignedQuantizer):
    def update_qparams(self,):
        ...
        float_range = float_range.item()
        # print("max v:{}".format(v))
        if (float_range < 1):
            si = 0
        else:
            si = math.ceil(math.log(float_range + 1e-12, 2))
            threshold = 2 ** (si - 1)
            ratio = torch.sum(self.observer.abs_value >= threshold).item() / self.observer.abs_value.size()[0]
            if ratio < 0.01:
                si = si - 1
        sf = self.bits - 1. - si
        scale = torch.tensor(math.pow(2.0, -sf))
        # scale = float_range / quant_range  # scale
        # scale = torch.max(scale, self.eps)  # processing for very small scale
        # scale1 = torch.log2(scale)
        # scale = pow(2, torch.round(torch.log2(scale)))
...
class QuantConv2d(nn.Conv2d):
        ...
        if not ptq:
            if q_type == 0:
                ...
                if weight_observer == 0:
                    if bias:
                        self.bias_quantizer = SymmetricQuantizer(
                            bits=a_bits,
                ...
                if weight_observer == 0:
                    if bias:
                        self.bias_quantizer = AsymmetricQuantizer(
                            bits=a_bits,
                            observer=MinMaxObserver(
                                q_level="L", out_channels=None
                            ),
    ...
    def forward(self, input):
        quant_input_int, quant_input_scale = self.activation_quantizer(input)
        quant_input = quant_input_int * quant_input_scale
        self.register_buffer("quant_input_s", quant_input_scale)
        if not self.quant_inference:
            quant_weight_int, quant_weight_scale = self.weight_quantizer(self.weight)
            self.register_buffer("quant_weight_int", quant_weight_int)
            self.register_buffer("quant_weight_s", quant_weight_scale)
            quant_weight = quant_weight_int * quant_weight_scale
        else:
            quant_weight = self.weight

        if self.bias is not None:
            quant_bias_int, quant_bias_scale = self.bias_quantizer(self.bias)
            self.register_buffer("quant_bias_int", quant_bias_int)
            self.register_buffer("quant_bias_s", quant_bias_scale)
            quant_bias = quant_bias_int * quant_bias_scale
        else:
...
class QuantDeformConvPack_d(DeformConvPack_d):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        super(QuantDeformConvPack_d, self).__init__(
        in_channels,
        out_channels,
        kernel_size,
        stride,
    ...
    def forward(self, input, ):
        ...
        if not self.quant_inference:
            ...
            self.register_buffer("quant_weight_s", quant_weight_scale)
            quant_weight = quant_weight_int * quant_weight_scale
            quant_offset_weight_int, quant_offset_weight_scale = self.conv_offset.weight_quantizer(self.conv_offset.weight)
            self.register_buffer("quant_offset_weight_int", quant_offset_weight_int)
            self.register_buffer("quant_offset_weight_s", quant_offset_weight_scale)
            quant_offset_weight = quant_offset_weight_int * quant_offset_weight_scale
        else:
            quant_weight = self.weight
            quant_offset_weight = self.conv_offset.weight
        ...
        if self.conv_offset.bias is not None:
            quant_offset_bias_int, quant_offset_bias_scale = self.conv_offset.bias_quantizer(self.conv_offset.bias)
            self.register_buffer("quant_offset_bias_int", quant_offset_bias_int)
            self.register_buffer("quant_offset_bias_s", quant_offset_bias_scale)
            quant_offset_bias = quant_offset_bias_int * quant_offset_bias_scale
        else:
            quant_offset_bias = self.conv_offset.bias

        quant_offset=F.conv3d(
            quant_input,
            quant_offset_weight,
            quant_offset_bias,
            self.stride,
        ...
        output = DeformConvFunction.apply(quant_input, offset,   #input(1,64,7,144,180) offset(1,27*3,7,144,180)
                                        quant_weight,
                                        quant_bias,
                                        self.stride,
                                        self.padding,
...
class QuantPixelShuffle(nn.PixelShuffle):
    def __init__(
        self,
        upscale_factor=4,
        a_bits=8,
        ...
        super(QuantPixelShuffle, self).__init__(
            upscale_factor
        )
        if not ptq:
            if q_type == 0:
...
class QuantAdd1(nn.Module):
    def __init__(self, a_bits=8, q_type=0, qaft=False, ptq=False, percentile=0.9999):
        super(QuantAdd1, self).__init__()
        if not ptq:
    ...
    def forward(self, res, quant_shortcut):
        self.observer_res(res)
        self.activation_quantizer.observer.min_val =  self.observer_res.min_val
        self.activation_quantizer.observer.max_val = self.observer_res.max_val
        ...
        self.activation_quantizer.observer.abs_value = res.abs().view(-1)
        quant_res_int,  quant_res_scale= self.activation_quantizer(res)
        self.register_buffer("quant_res_s", quant_res_scale)
        quant_res = quant_res_int * quant_res_scale
        # quant_shortcut = self.activation_quantizer(shortcut)
        output = quant_res + quant_shortcut
        return output

class QuantAdd2(nn.Module):
    def __init__(self, a_bits=8, q_type=0, qaft=False, ptq=False, percentile=0.9999):
        super(QuantAdd2, self).__init__()
        if not ptq:
            self.observer_res = MovingAverageMinMaxObserver(
                q_level="L", out_channels=None
            )
    ...
    def forward(self, res, shortcut):
        ...
        self.register_buffer("quant_res_s", quant_res_scale)
        quant_res = quant_res_int * quant_res_scale
        quant_shortcut_int,  quant_shortcut_scale= self.activation_quantizer(shortcut)
        self.register_buffer("quant_shortcut_s", quant_shortcut_scale)
        quant_shortcut = quant_shortcut_int * quant_shortcut_scale
        output = quant_res + quant_shortcut
        return output
...
def add_quant_op(
    ...
    for name, child in module.named_children():
        if isinstance(child, nn.Conv2d):
            if bn_fuse:
                ...
                if child.bias is not None:
                    ...
                    quant_conv2d.bias.data = child.bias
                else:
                    quant_conv2d = QuantConv2d(
                        child.in_channels,
                        child.out_channels,
                ...
                quant_conv2d.weight.data = child.weight
                module._modules[name] = quant_conv2d
            ...
            if bn_fuse:
                ...
                if child.bias is not None:
                    ...
                    quant_conv3d.bias.data = child.bias
                else:
                    quant_conv3d = QuantConv3d(
                        child.in_channels,
                        child.out_channels,
                ...
                quant_conv3d.weight.data = child.weight
                module._modules[name] = quant_conv3d
        elif isinstance(child,DeformConvPack_d):
            if bn_fuse:
                conv_name_temp = name
                conv_child_temp = child
            else:
                if child.bias is not None:
                    quant_deform_conv = QuantDeformConvPack_d(
                        child.in_channels,
                        child.out_channels,
                        child.kernel_size,
                        stride=child.stride,
                        ...
                        dilation=child.dilation,
                        groups=child.groups,
                        deformable_groups=child.deformable_groups,
                        im2col_step=child.im2col_step,
                        bias=True,
                        a_bits=a_bits,
                        w_bits=w_bits,
                        q_type=q_type,
                    ...
                    quant_deform_conv.bias.data = child.bias
                else:
                    quant_deform_conv = QuantDeformConvPack_d(
                        child.in_channels,
                        child.out_channels,
                ...
                quant_deform_conv.weight.data = child.weight
                module._modules[name] = quant_deform_conv
                if isinstance(child.conv_offset, nn.Conv3d):
                    if child.conv_offset.bias is not None:
                        quant_offset_conv = QuantConv3d(
                            child.conv_offset.in_channels,
                            child.conv_offset.out_channels,
                            child.conv_offset.kernel_size,
                            stride=child.conv_offset.stride,
                            padding=child.conv_offset.padding,
                            dilation=child.conv_offset.dilation,
                            groups=child.conv_offset.groups,
                            bias=True,
                            padding_mode=child.conv_offset.padding_mode,
                            a_bits=a_bits,
                            w_bits=w_bits,
                            q_type=q_type,
                            q_level=q_level,
                        ...
                        quant_offset_conv.bias.data = child.conv_offset.bias
                    else:
                        quant_offset_conv = QuantConv3d(
                            child.conv_offset.in_channels,
                            child.conv_offset.out_channels,
                    ...
                    quant_offset_conv.weight.data = child.conv_offset.weight
                    module._modules[name].conv_offset = quant_offset_conv
        ...
        #    module._modules[name] = quant_relu
            ...
            quant_pixelshuffle = QuantPixelShuffle(
                upscale_factor=child.upscale_factor,
                a_bits=a_bits,
                q_type=q_type,
                qaft=qaft,
                ptq = ptq,
            ...
            module._modules[name] = quant_pixelshuffle
        elif isinstance(child, Add1):
            quant_add = QuantAdd1(
                a_bits=a_bits, q_type=q_type, qaft=qaft, ptq=ptq, percentile=percentile
            )
            module._modules[name] = quant_add
        elif isinstance(child, Add2):
            quant_add = QuantAdd2(
                a_bits=a_bits, q_type=q_type, qaft=qaft, ptq=ptq, percentile=percentile
            )
            module._modules[name] = quant_add
